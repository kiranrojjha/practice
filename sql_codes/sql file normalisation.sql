...
CREATE TABLE books_bcnf (
    book_isbn VARCHAR(20) PRIMARY KEY,
    book_title VARCHAR(200),
    book_author VARCHAR(100),
    book_price DECIMAL(10, 2)
);
CREATE TABLE order_items_bcnf (
    order_id INT,
    book_isbn VARCHAR(20),
    quantity INT,
    total_price DECIMAL(10, 2),
    PRIMARY KEY (order_id, book_isbn),
    FOREIGN KEY (order_id) REFERENCES orders_bcnf(order_id),
    FOREIGN KEY (book_isbn) REFERENCES books_bcnf(book_isbn)
);
INSERT INTO orders_bcnf VALUES
(1, 'John Smith', 'john@example.com', '123 Main St, Anytown', '2023-01-15'),
(2, 'John Smith', 'john@example.com', '123 Main St, Anytown', '2023-01-15'),
(3, 'Mary Johnson', 'mary@example.com', '456 Oak Ave, Somewhere', '2023-01-20'),
(4, 'Robert Brown', 'robert@example.com', '789 Pine Rd, Nowhere', '2023-01-25');
;
INSERT INTO books_bcnf VALUES
('978-0141439518', 'Pride and Prejudice', 'Jane Austen', 9.99),
('978-0451524935', '1984', 'George Orwell', 12.99),
('978-0061120084', 'To Kill a Mockingbird', 'Harper Lee', 14.99);
INSERT INTO order_items_bcnf VALUES
(1, '978-0141439518', 1, 9.99),
(2, '978-0451524935', 2, 25.98),
(3, '978-0061120084', 1, 14.99),
(4, '978-0141439518', 1, 9.99);
